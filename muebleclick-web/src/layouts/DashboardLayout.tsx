      ...
      {items.map((item) => (
        <NavLink
          ...
          className={({ isActive }) => cn(
            ...
            isActive
              ? 'bg-(--color-primary) text-white shadow-md shadow-green-900/20'
              : 'text-muted-foreground hover:bg-white hover:text-(--color-foreground) hover:shadow-sm',
          )}
        >
...
function Sidebar() {
  ...
  return (
    <aside className="w-64 h-full flex flex-col"
      style={{ background: 'linear-gradient(180deg, #F5F0E8 0%, #EDE0CC 100%)', borderRight: '1px solid #E2D9CE' }}>
      ...
      <div className="flex items-center gap-3 px-5 py-5">
        ...
        <div>
          <p className="font-bold text-primary-dark text-base leading-none">
            MuebleClick
          </p>
      ...
      <div className="mx-3 h-px bg-(--color-border)" />

      {/* Nav */}
      <div className="flex-1 overflow-y-auto py-3">
        <p className="px-6 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-2">
          Menú principal
        </p>
        <NavItems />
      </div>
      ...
      <div className="p-3">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/60 border border-white/80 shadow-sm">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold truncate text-(--color-foreground)">
              {usuario?.nombre}
            </p>
        ...
        <header className="h-14 glass border-b flex items-center justify-between px-4 lg:px-6 shrink-0">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="flex items-center gap-2 h-9 px-3 rounded-xl hover:bg-white/60">
            ...
            <DropdownMenuContent align="end" className="w-52 rounded-xl shadow-xl border-(--color-border)">
